		...
		*/
		int GetRowCount() const override { return Matrix<T>::rows_; }
		  ...
		  T GetElement(int i, int j) const override {
		    if(i>=Matrix<T>::rows_||j>=Matrix<T>::cols_)
...
class RowMatrixOperations {
	public:
	    ...
	    static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
	      // TODO(P0): Add implementation
	    	int row=matrixA->GetRowCount(),col=matrixA->GetColumnCount();
	    	if(row!=matrixB->GetRowCount()||col!=matrixB->GetColumnCount())
	        	return std::unique_ptr<RowMatrix<T>>(nullptr);
	    	RowMatrix<T> * C=new RowMatrix<T>(row,col);
	    	for(int i = 0; i < row; i++){
	        	for(int j = 0; j < col; j++){
	            	C->SetElement(i,j,matrixA->GetElement(i,j)+matrixB->GetElement(i,j));
	        	}
	    	}
	    	std::unique_ptr<RowMatrix<T>> t(C);
	    	return t; 
	    }
	    ...
	    static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
	      // TODO(P0): Add implementation
	    	...
	    	T t1=matrixA->GetColumnCount(),t2= matrixB->GetRowCount();
	    	if(t1!=t2)
	    		return std::unique_ptr<RowMatrix<T>>(nullptr);
	    	RowMatrix<T> * C=new RowMatrix<T>(row,col);
	    	for(int i = 0; i < row; i++){
	    ...
	    static std::unique_ptr<RowMatrix<T>> GEMM(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB,
	      ...
	      // TODO(P0): Add implementation
	    	std::unique_ptr<RowMatrix<T>> t=Multiply(matrixA,matrixB);
	    	if(t==nullptr)
	        	return std::unique_ptr<RowMatrix<T>>(nullptr);
	    	t=Add(t.get(),matrixC);
	    	if(t==nullptr){
